...
def application_form(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            message = render_to_string('confirmation_email.html', {'user': request.user})
...
class UpdatePostView(UpdateView):
    ...
    def form_valid(self, form):
        ...
        if self.object.Application_Status == 'Approved':
            # Send an email to the user
            subject = 'Application Approval Notification'
            message = render_to_string('approval_email.html', {'user': self.object.user})
            plain_message = strip_tags(message)
            from_email = settings.DEFAULT_FROM_EMAIL
            to_email = [self.object.user.email]
            send_mail(subject, plain_message, from_email, to_email, html_message=message)
